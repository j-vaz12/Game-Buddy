...
async function addGameToUser(req, res) {
    try {
        ...
        console.log(req.body, "this is line 25 BRO")
        let currGame; 
        ...
        let userGameInDb = await UserGame.exists({user: req.user._id, game: currGame._id})
        if (!userGameInDb) await UserGame.create({ user: req.user._id, game: currGame._id })
